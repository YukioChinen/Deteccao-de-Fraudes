# CatBoost para detecção de fraudes com otimizações
...
# 6. Treinamento com CatBoost
print("Treinando modelo CatBoost...")

# Parâmetros otimizados para CatBoost
params = {
    'iterations': 5000,
    'learning_rate': 0.01,
    'depth': 8,
    ...
    'verbose': 100,
    'task_type': 'CPU', # Mudar para 'GPU' se disponível
    'bootstrap_type': 'Bayesian',
    'scale_pos_weight': sum(y_train_res == 0) / sum(y_train_res == 1),  # Balanceamento de classes
}

# Preparando conjuntos de dados para CatBoost
...
plt.tight_layout()
plt.savefig('catboost_results.png')
plt.show()
...
print("Modelo salvo em 'catboost_fraud_model.cbm'")

"""
O CatBoost é um algoritmo de gradient boosting desenvolvido pela Yandex que possui várias vantagens:

1. Lida naturalmente com variáveis categóricas sem necessidade de pré-processamento
2. Implementa técnicas para evitar overfitting durante o treinamento
3. Bom desempenho com conjuntos de dados desbalanceados
4. Implementação eficiente e paralelizável
5. Geralmente requer menos ajuste de hiperparâmetros que outros algoritmos

Para detecção de fraudes, o CatBoost geralmente apresenta:
- Excelente capacidade discriminativa (AUC)
- Bom equilíbrio entre precisão e recall
- Robustez contra overfitting mesmo com dados altamente desbalanceados
